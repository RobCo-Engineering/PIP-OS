    ...
    , m_player(new QMediaPlayer(this))
{
    ...
    connect(m_player, &QMediaPlayer::positionChanged, this, &Radio::positionChanged);
    connect(watchdogTimer, &QTimer::timeout, this, &Radio::checkPlayback);

    connect(m_player, &QMediaPlayer::playingChanged, this, &Radio::playingChanged);
    ...
    m_sfxRadioOn.setSource(QUrl("qrc:/assets/sounds/radio_on.wav"));
    m_sfxRadioOff.setSource(QUrl("qrc:/assets/sounds/radio_off.wav"));
}
...
void Radio::checkPlayback()
{
    ...
    // playing
    if (now - lastPositionUpdate > 1000 && m_player->playbackState() == QMediaPlayer::PlayingState) {
